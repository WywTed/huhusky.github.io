...
* 内部类 
    * ModuleConfiguration: 模块配置类
	  持有 模块提供者的配置引用： HashMap<String, ProviderConfiguration> providers
    * ProviderConfiguration:  模块提供者的 配置
	  持有 Properties 对象的引用， 表示该 provider 的配置，最终都转化为 Properties 对象
    
BootstrapFlow: 启动流程


—————————————————————— 综上：
1. skywalking 分模块管理， 有多个模块，
  每个 module 都有一个 provider， 每个 provider 可以注册多个 Service, 而 Service 可以理解为 具体功能的实现； 与 Provider 一起工作；
  
  
Skywalking 服务端启动流程：
  apm-dist apm-dist-es7 中表示发布时使用哪个启动类， 如果 es7 则
  对应启动为 oap-server/server-starter  oap-server/server-starter-es7, 实际最终都是以 opa-server/server-bootstrap 模块启动： OAPServerBootstrap#start()
...
OAPServerBootstrap.start():
  ...
  2. 构造配置加载类， ApplicationConfigLoader， 构造模块管理器 ModuleManager
      在这个地方， ApplicationConfigLoader 实现于 ConfigLoader<T> , 其中 T 的具体类型为 ApplicationConfiguration， 其中定义 load 方法，表示如何加载配置文件得到 T， 
	...
	2.2 然后调用模块管理器 manager 的 init 方法使用加载获取到的 ApplicationConfiguration 进行初始化
      ...
      2.2.5 通过加载的 loadedModules 构造 BootstrapFlow， 执行 bootstrapFlow 的 start（moduleManager） ，然后执行通知
        ...
        2.2.5.3 BootstrapFlow 的 notifyAfterComplete
          2.2.5.3.1 遍历 startupSequence 链表，依次执行每个 provider 的 notifyAfterCompleted() 方法
...
综上所述：
  ...
  3. 构造 bootstrapFlow 然后捋清 provider 的依赖形成一个具有先后顺序链表，
     ...
     最后再遍历链表依次执行每个 provider 的 notifyComplete() 方法

接下来依次分析各个 provider 的逻辑
...
ClusterModuleNacosProvider:  属于 clusterModule 的一个 provider, 主要用来作 skywalking 的集群管理
  ...
  start:
    ...
    2. 通过 esClient 构造 StorageEsInstaller 并添加到 coreModule 中 ModelCreator 服务的 modelListener 列表中去
        ...
        在 ModelInstaller 中的 whenCreating 方法中定义： 
          如果 启动模式 Mode 不为 "no-init", 则创建表结构（初始化相关， es 的索引， mysql 的数据表）
          如果启动模式 Mode 带有 "no-init",表示不做初始化动作，并循环停顿 3s 检查表接口是否存在.
    notifyAfterCompleted: do nothing.
...
JVMModuleProvider:
  ...
  start:
    1. 从 coreModule 中拿到 OALEngineLoaderService 并加载 JVMOALDefine.INSTANCE 主要就是 加载关于 jvm 的 oal
       "oal/java-agent.oal" 文件，另外就是 "org.apache.skywalking.oap.server.core.source" 包下
...
综上可以看出
  ...
  2. grpcHandlerRegister 和 jettyHandlerRegister 做着承上启下的作用，从头到尾都在添加 Handler 处理器，
    ...
    所以比较核心的就是要了解 模块中的这些 Handler 做了些什么事情 